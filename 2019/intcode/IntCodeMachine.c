...
void icm__increment_ip(IntCodeMachine_t *icm, int instruction){
    if (instruction == ADD_OP) icm->instruction_pointer += 4;
    else if (instruction == MULT_OP) icm->instruction_pointer += 4;
}
void icm__perform(IntCodeMachine_t *icm){
    ...
    while (icm->opcodes[icm->instruction_pointer] != HALT_OP){
        if (icm->opcodes[icm->instruction_pointer] == ADD_OP){
        ...
        else if (icm->opcodes[icm->instruction_pointer] == MULT_OP){
            operand1 = icm->opcodes[icm->instruction_pointer + 1];
            operand2 = icm->opcodes[icm->instruction_pointer + 2];
            location = icm->opcodes[icm->instruction_pointer + 3];
        ...
        icm__increment_ip(icm, icm->opcodes[icm->instruction_pointer]);
    }
}
